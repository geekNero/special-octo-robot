...
def update_table(table: str, new_data: dict) -> None:
    ...
    """
    set_clause = ", ".join([f"{key} = ?" for key in new_data.keys() if key != "id"])
    values = [value for key, value in new_data.items() if key != "id"]
    values.append(new_data["id"])
...
def rename_table(old_table_name: str, new_table_name: str) -> None:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        try:
            ...
            cur.execute(
                f"CREATE INDEX title_{new_table_name} on {new_table_name}(title);",
            )
...
def add_session(
    ...
    with sqlite3.connect(db_path) as conn:
        cur = conn.cursor()
        cur.execute(query, (task_id, table_name, start_datetime, end_datetime))
...
def add_session_data(
    ...
    with sqlite3.connect(db_path) as conn:
        cur = conn.cursor()
        cur.execute(query, (session_id, application_name, duration))
...
def list_sessions(table_name: str, task_id: int = None) -> list:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        if task_id is not None:
            ...
            query = f"SELECT * FROM sessions WHERE table_name = ? ORDER BY end_datetime DESC"
